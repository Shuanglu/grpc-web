...
func main() {
	...
	wg.Add(1)
	go grpc_run(fmt.Sprintf("%s:%d", *client_server_addr, *client_grpc_port), *client_header_host, mesh, ip, *role, client_request_total)
	wg.Add(1)
	go http_run(fmt.Sprintf("http://%s:%d", *client_server_addr, *client_http_port), *client_header_host, mesh, ip, *role, client_request_total)
	wg.Wait()
}

func grpc_run(dest string, host string, mesh string, ip string, role string, client_request_total *int) {
	if role == "server" {
		grpcserver.Run(server_grpc_port, version, mesh, ip)
	} else if role == "client" {
		if *client_grpc {
			grpcclient.Run(dest, host, mesh, ip, *client_request_total)
		}
	} else {
...
func http_run(dest string, host string, mesh string, ip string, role string, client_request_total *int) {

	if role == "server" {
		webserver.Run(server_http_port, version, mesh, ip, dest, host)
	} else if role == "client" {
		if *client_http {
			go webclient.Run(dest, host, mesh, ip, *client_request_total)
		}
	} else {
